...
mkdir x64\release
pushd x64\release
cmake -DCMAKE_INSTALL_PREFIX=install -DBUILD_TESTS=ON -DCMAKE_PREFIX_PATH=%SRC%\copperspice-libaries\lib\cmake\Catch2 -DCMAKE_BUILD_TYPE=Release -G Ninja %SRC%
popd

mkdir x64\debug
pushd x64\debug
cmake -DCMAKE_INSTALL_PREFIX=install -DBUILD_TESTS=ON -DCMAKE_PREFIX_PATH=%SRC%\copperspice-libaries\lib\cmake\Catch2 -DCMAKE_BUILD_TYPE=Debug -G Ninja %SRC%
popd
